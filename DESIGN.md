...
**Mascot**: 🦉 *Le Veilleur* — navy owl with amber eyes, Pixar style, 16:9. Generated by Imagen 4 Fast per article; never used as a UI illustration outside the hero image of a published article. The header glyph in the PWA shell is the emoji `🦉` on an amber-tinted circle (see Astro `BaseLayout`). The same emoji glyph is the PWA's favicon and app icon (`pwa/public/icons/icon*.svg`, `apple-touch-icon.png`) — rendered onto the brand navy square, not the Imagen illustration, so it stays one glyph everywhere it appears in chrome.
