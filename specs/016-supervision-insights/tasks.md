...
- [x] **T-1.1**: `computeTrends` pure function
...
- [x] **T-1.2**: Unit tests for `computeTrends`
...
- [x] **T-2.1**: `classifyFailure` + `parseInsufficientSources` in `runErrors.ts`
...
- [x] **T-2.2**: Unit tests for `classifyFailure`/`parseInsufficientSources`
...
- [x] **T-2.3**: Wire structured diagnosis into `RunTimeline`'s `ErrorBanner`
...
- [x] **T-3.1**: Render `step.error` in `RunStepRow`
...
- [x] **T-4.1**: Add the trends section to `Supervision.tsx`
...
- [x] **T-4.2**: Extend `supervision.test.tsx` with trends + diagnosis cases
...
- [x] **T-4.3**: Full PWA regression gate
