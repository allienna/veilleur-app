...
1. **Single allowed identity.** Only the hardcoded operator email may read or write Firestore, call `trigger-api`, or read the PWA. Defense-in-depth: Firestore Security Rules + trigger-api JWT verification (`email_verified == true`). The PWA client check is UX only, never the security boundary.
2. **OAuth-only Anthropic auth.** `ANTHROPIC_API_KEY` is **absent from env by default**. The container authenticates via `CLAUDE_CODE_OAUTH_TOKEN`. The API-key fallback exists in Secret Manager but is mounted only by explicit, manually-edited deployment, never automated.
...
| Agentic step | `claude -p "/generate"` against versioned `.claude/commands/generate.md` | **Locked** |
...
- **Slash commands are production code**: `minion/.claude/commands/generate.md` is versioned, reviewed, and changes require a PR — treat it like the orchestrator.
